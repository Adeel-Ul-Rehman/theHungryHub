...
const initialMaterials = [
  ...
  { id: 5, name: 'Pizza Mozzarella Cheese', category: 'Dairy', stock: 1800, unit: 'grams (g)', min: 2000, cost: 1.8 }, // LOW STOCK
  { id: 6, name: 'French Fries (Frozen)', category: 'Vegetables', stock: 20000, unit: 'grams (g)', min: 5000, cost: 0.6 },
  { id: 7, name: 'Garlic Mayo Sauce', category: 'Sauces & Spices', stock: 800, unit: 'milliliters (ml)', min: 1000, cost: 0.5 }, // LOW STOCK
  { id: 8, name: 'Cooking Oil', category: 'Pantry', stock: 15000, unit: 'milliliters (ml)', min: 3000, cost: 0.4 },
...
const InventoryPage = () => {
  ...
  const handleExportCSV = () => {
    const exportData = filteredMaterials.map(m => ({
      ...
      MinThreshold: m.min,
      UnitCostPKR: m.cost,
      TotalValuationPKR: m.stock * m.cost
    }));
  ...
  const handleExportPDF = () => {
    ...
    const rows = filteredMaterials.map(m => [
      ...
      `PKR ${(m.stock * m.cost).toLocaleString()}`
    ]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
          <h2 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2">
            <Boxes className="w-6 h-6 text-orange-500" /> Domino's & KFC Style Raw Material Inventory
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Live stock levels, alert thresholds, and multi-format reports export</p>
        </div>
              ...
              {filteredMaterials.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="py-4 px-4 font-bold text-slate-900 dark:text-white">{item.name}</td>
                    <td className="py-4 px-4 font-medium text-slate-500 dark:text-slate-400">{item.category}</td>
                    <td className="py-4 px-4 font-extrabold text-slate-900 dark:text-white">
                      {item.stock.toLocaleString()} <span className="text-xs font-normal text-slate-400">{item.unit}</span>
                      {/* Mini Stock Progress Bar */}
                      <div className="w-28 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full mt-1.5 overflow-hidden">
                        <div 
                    ...
                    <td className="py-4 px-4 font-semibold text-slate-500">{item.min.toLocaleString()} {item.unit}</td>
                    <td className="py-4 px-4">
                      {isOut ? (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-rose-100 text-rose-700 dark:bg-rose-950/60 dark:text-rose-400">
                          <XCircle className="w-3.5 h-3.5" /> OUT OF STOCK
                        </span>
                      ) : isLow ? (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-amber-100 text-amber-700 dark:bg-amber-950/60 dark:text-amber-400">
                          <AlertTriangle className="w-3.5 h-3.5" /> LOW STOCK
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-400">
                    ...
                    <td className="py-4 px-4 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 font-bold text-xs hover:bg-orange-100 transition-colors"
                      >
                        + Restock
                      </button>
                    </td>
      ...
      {restockModalItem && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6">
            <h3 className="text-lg font-extrabold text-slate-900 dark:text-white">📦 Restock {restockModalItem.name}</h3>
